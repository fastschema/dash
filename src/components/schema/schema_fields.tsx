...
import { defaultFieldValues, schemaSchema } from './data';
...
export interface SchemaFieldsProps {
  ...
  setEditingField: Dispatch<SetStateAction<Field | undefined>>;
  removeField: UseFieldArrayRemove;
}

export const SchemaFields = (props: SchemaFieldsProps) => {
  const { form, fields, setEditingField: setEdittingField, removeField } = props;
        ...
        <TableBody>
          {fields.map((field, index) => (
            <TableRow key={field.name}>
              ...
              <TableCell className="font-medium truncate">{field.name}</TableCell>
              ...
              <TableCell>{field.is_system_field ? <Badge variant="secondary">System</Badge> : null}</TableCell>
              <TableCell>{!field.optional ? <Badge variant="destructive">Required</Badge> : null}</TableCell>
